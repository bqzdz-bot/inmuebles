...
export function MoneyInput({label,error,className,value,onChange,...p}){const display=value?Number(value).toLocaleString('es-MX'):'';const handleChange=(e)=>{const raw=e.target.value.replace(/[^0-9.-]/g,'');onChange({target:{value:raw}})};return<div className={clsx('space-y-1',className)}>{label&&<label className="block text-[12px] font-medium text-slate-600">{label}</label>}<input className={clsx('block w-full rounded-lg border border-slate-200 px-3 py-2 text-[13px] focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100',error&&'border-red-300')} value={display} onChange={handleChange} {...p}/>{error&&<p className="text-[11px] text-red-500">{error}</p>}</div>}
