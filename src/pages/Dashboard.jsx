  ...
  return <div>
    ...
    <Card className="mb-5">
      ...
      <div className="divide-y divide-slate-50">
        {groups.data.map(g=>{
          ...
          const gRenta=gProps.filter(p=>p.estatus==='rentada').reduce((s,p)=>s+Number(p.renta),0)
    ...
    <div className="grid grid-cols-5 gap-3 mb-5">
      <div className="col-span-3"><Card>
        ...
        <div className="p-4 h-56"><ResponsiveContainer width="100%" height="100%"><BarChart data={INCOME_CHART} barGap={4}><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false}/><XAxis dataKey="mes" tick={{fontSize:11,fill:'#94a3b8'}} axisLine={false} tickLine={false}/><YAxis tick={{fontSize:11,fill:'#94a3b8'}} axisLine={false} tickLine={false} tickFormatter={v=>`$${Math.round(v/1000)}k`}/><Tooltip formatter={v=>fmt(v)} contentStyle={{borderRadius:8,border:'1px solid #e2e8f0',fontSize:12}}/><Bar dataKey="ingresos" name="Ingresos" fill="#2563EB" radius={[4,4,0,0]} barSize={28}/></BarChart></ResponsiveContainer></div>
      </Card></div>
    ...
    <Card><CardHeader title="Alertas"/><div className="p-3 space-y-2">
      ...
      {maintenance.data.filter(m=>m.es_recurrente&&m.prox_ejecucion).length>0&&<div className="flex gap-3 items-start p-3 rounded-lg bg-blue-50"><div className="w-6 h-6 rounded-full bg-blue-500 flex items-center justify-center flex-shrink-0"><Info className="w-3 h-3 text-white"/></div><div><p className="text-[12px] font-medium text-slate-700">{maintenance.data.filter(m=>m.es_recurrente).length} servicio(s) recurrentes programados</p></div></div>}
    </div></Card>
  </div>
}
